...
class lactate_solver(object):
    def __init__(self, lactate_vector, heartrate_vector ,external_target_vector):
    ...
    def method_2and4mmol(self):
        LT1_mmol = LT1_external_target = LT1_hr = LT2_mmol = LT2_external_target = LT2_hr = LTfit = 0
    ...
    def plotter(self, LT1_mmol, LT1_external_target, LT1_hr, LT2_mmol, LT2_external_target, LT2_hr, LTfit, save=False):
        x_new = np.linspace(start=self.external_target_vector.min() ,stop=self.external_target_vector.max() ,num=200)
        lac_fit = LTfit(x_new)
